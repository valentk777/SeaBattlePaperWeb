...
public sealed class SeaBattleService(ISeaBattleStore store, TimeProvider timeProvider)
{
   ...
   private static readonly IReadOnlyCollection<IReadOnlyCollection<BoardCoordinate>> ClassicCandidateShapes = CreateClassicCandidateShapes();
   private static readonly IReadOnlyCollection<IReadOnlyCollection<BoardCoordinate>> PaperCandidateShapes = CreatePaperCandidateShapes();
       ...
       IReadOnlySet<Guid> onlinePlayers)
   {
       ...
       var myBoard = BuildBoard(viewer, opponentShots, true, mode: match.Mode);
       var opponentBoard = opponent is null
           ? Array.Empty<CellStateDto>()
           : BuildBoard(opponent, viewerShots, match.Status == MatchStatus.Finished, match.RevealSunkShips, match.Mode);
       ...
       string mode = "classic")
   {
       ...
       if (!revealAllShips && !revealSunkShips)
           foreach (var water in InferImpossibleWater(mode, incomingShots, visibleCells.Keys).Where(water => !visibleCells.ContainsKey(water)))
               visibleCells[water] = new CellStateDto(water.Row, water.Column, false, ShotResult.Miss, null);
       ...
       IEnumerable<BoardCoordinate> visibleCells)
   {
       ...
       for (var row = 0; row < FleetRules.BoardSize; row++)
       {
           for (var column = 0; column < FleetRules.BoardSize; column++)
           {
               ...
               if (visible.Contains(cell)) continue;
               if (!CouldAnyShipOccupy(cell, mode, knownHits, knownMisses)) yield return cell;
           }
       }
   }


   private static bool CouldAnyShipOccupy(
       BoardCoordinate target,
       string mode,
       IReadOnlySet<BoardCoordinate> knownHits,
       IReadOnlySet<BoardCoordinate> knownMisses)
   {
       var candidateShapes = string.Equals(mode, "paper", StringComparison.OrdinalIgnoreCase)
           ? PaperCandidateShapes
           : ClassicCandidateShapes;


       foreach (var shape in candidateShapes)
       {
           foreach (var targetOffset in shape)
           {
               var start = new BoardCoordinate(target.Row - targetOffset.Row, target.Column - targetOffset.Column);
               var cells = shape
                   .Select(offset => new BoardCoordinate(start.Row + offset.Row, start.Column + offset.Column))
                   .ToHashSet();


               if (cells.Any(cell => !FleetRules.IsInside(cell.Row, cell.Column))) continue;
               if (cells.Overlaps(knownMisses)) continue;


               var touchesKnownHitOutsideCandidate = knownHits
                   .Where(hit => !cells.Contains(hit))
                   .Any(hit => cells.Any(candidate => Touches(candidate, hit)));
               if (touchesKnownHitOutsideCandidate) continue;
   ...
   private static bool Touches(BoardCoordinate left, BoardCoordinate right) =>
       Math.Abs(left.Row - right.Row) <= 1 && Math.Abs(left.Column - right.Column) <= 1;
