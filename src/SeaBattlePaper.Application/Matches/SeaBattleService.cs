...
public sealed class SeaBattleService(ISeaBattleStore store, TimeProvider timeProvider)
{
    ...
    private static readonly IReadOnlyDictionary<int, int> RequiredShipCounts = new Dictionary<int, int>
    {
        [1] = 4,
        [2] = 3,
        [3] = 2,
    ...
    private static readonly IReadOnlyCollection<CandidatePlacement> ClassicCandidatePlacements = CreateCandidatePlacements(CreateClassicCandidateShapes());
    private static readonly IReadOnlyCollection<CandidatePlacement> PaperCandidatePlacements = CreateCandidatePlacements(CreatePaperCandidateShapes());
        ...
        IReadOnlySet<Guid> onlinePlayers)
    {
        ...
        var myBoard = BuildBoard(viewer, opponentShots, true, mode: match.Mode, inferImpossibleWater: true);
        var opponentBoard = opponent is null
            ? Array.Empty<CellStateDto>()
            : BuildBoard(opponent, viewerShots, match.Status == MatchStatus.Finished, match.RevealSunkShips, match.Mode, true);
    ...
    private static IReadOnlyCollection<CellStateDto> BuildBoard(
        ...
        string mode = "classic",
        bool inferImpossibleWater = false)
    {
        ...
        var sunkShips = owner.Ships
            ...
            .ToArray();
        var confirmedSunkShips = GetConfirmedSunkShips(sunkShips, incomingShots, mode);
        ...
        if (inferImpossibleWater)
            foreach (var water in InferImpossibleWater(mode, incomingShots, confirmedSunkShips, visibleCells.Keys).Where(water => !visibleCells.ContainsKey(water)))
                visibleCells[water] = new CellStateDto(water.Row, water.Column, false, ShotResult.Miss, null);

        return visibleCells.Values.OrderBy(cell => cell.Row).ThenBy(cell => cell.Column).ToArray();
    }

    private static IEnumerable<BoardCoordinate> InferImpossibleWater(
        string mode,
        IReadOnlyCollection<Shot> incomingShots,
        IReadOnlyCollection<Ship> sunkShips,
        IEnumerable<BoardCoordinate> visibleCells)
    {
        var visible = visibleCells.ToHashSet();
        var knownHits = incomingShots
            .Where(shot => shot.Result != ShotResult.Miss)
            .Select(shot => new BoardCoordinate(shot.Row, shot.Column))
            .ToHashSet();
        if (knownHits.Count == 0) yield break;

        var knownMisses = incomingShots
            .Where(shot => shot.Result == ShotResult.Miss)
            .Select(shot => new BoardCoordinate(shot.Row, shot.Column))
            .ToHashSet();
        var remainingLengths = GetRemainingShipLengths(sunkShips);
        if (remainingLengths.Count == 0) yield break;

        var possibleOccupiedCells = new HashSet<BoardCoordinate>();
        foreach (var placement in GetCandidatePlacements(mode))
        {
            if (!remainingLengths.Contains(placement.Length)) continue;
            if (placement.Cells.Overlaps(knownMisses)) continue;
            if (!PlacementCanCoexistWithKnownHits(placement, knownHits)) continue;

            possibleOccupiedCells.UnionWith(placement.Cells);
        }

        for (var row = 0; row < FleetRules.BoardSize; row++)
        {
            for (var column = 0; column < FleetRules.BoardSize; column++)
            {
                var cell = new BoardCoordinate(row, column);
                if (!visible.Contains(cell) && !possibleOccupiedCells.Contains(cell))
                    yield return cell;
            }
        }
    }

    private static IReadOnlyCollection<Ship> GetConfirmedSunkShips(
        IReadOnlyCollection<Ship> sunkShips,
        IReadOnlyCollection<Shot> incomingShots,
        string mode)
    {
        var misses = incomingShots
            .Where(shot => shot.Result == ShotResult.Miss)
            .Select(shot => new BoardCoordinate(shot.Row, shot.Column))
            .ToHashSet();
        var hits = incomingShots
            ...
            .Select(shot => new BoardCoordinate(shot.Row, shot.Column))
            .ToHashSet();
        var confirmed = new List<Ship>();
        var remaining = sunkShips.ToHashSet();
        var changed = true;

        while (changed)
        {
            changed = false;
            var remainingLengths = GetRemainingShipLengths(confirmed);
            foreach (var ship in remaining
                         .Where(ship => !CanSunkShipStillBeLarger(ship, mode, hits, misses, remainingLengths))
                         .ToArray())
            {
                confirmed.Add(ship);
    ...
    private static bool CanSunkShipStillBeLarger(
        Ship ship,
        string mode,
        IReadOnlySet<BoardCoordinate> knownHits,
        IReadOnlySet<BoardCoordinate> knownMisses,
        IReadOnlySet<int> remainingLengths)
    {
        var cells = ship.Cells().ToHashSet();
        if (cells.Count >= 4) return false;

        foreach (var length in remainingLengths.Where(length => length > cells.Count))
        {
            foreach (var placement in GetCandidatePlacements(mode).Where(placement => placement.Length == length))
            {
                if (!cells.IsSubsetOf(placement.Cells)) continue;
                if (placement.Cells.Overlaps(knownMisses)) continue;
                if (!PlacementCanCoexistWithKnownHits(placement, knownHits)) continue;
    ...
    private static IReadOnlySet<int> GetRemainingShipLengths(IReadOnlyCollection<Ship> confirmedSunkShips) =>
        RequiredShipCounts
            .Where(required => confirmedSunkShips.Count(ship => ship.Length == required.Key) < required.Value)
            .Select(required => required.Key)
            .ToHashSet();

    private static bool PlacementCanCoexistWithKnownHits(
        CandidatePlacement placement,
        IReadOnlySet<BoardCoordinate> knownHits) =>
        knownHits
            .Where(hit => !placement.Cells.Contains(hit))
            .All(hit => !placement.TouchingCells.Contains(hit));

    private static IReadOnlyCollection<CandidatePlacement> GetCandidatePlacements(string mode) =>
        string.Equals(mode, "paper", StringComparison.OrdinalIgnoreCase)
            ? PaperCandidatePlacements
            : ClassicCandidatePlacements;

    private static IReadOnlyCollection<CandidatePlacement> CreateCandidatePlacements(
        IReadOnlyCollection<IReadOnlyCollection<BoardCoordinate>> shapes)
    {
        var placements = new List<CandidatePlacement>();
        foreach (var shape in shapes)
        {
            for (var row = 0; row < FleetRules.BoardSize; row++)
            {
                ...
                {
                    var cells = shape
                        .Select(offset => new BoardCoordinate(row + offset.Row, column + offset.Column))
                        .ToHashSet();
                    if (cells.Any(cell => !FleetRules.IsInside(cell.Row, cell.Column))) continue;

                    var touchingCells = cells
                        .SelectMany(TouchingCoordinates)
                        .ToHashSet();
                    placements.Add(new CandidatePlacement(cells.Count, cells, touchingCells));
                }
            }
        }
    ...
    private static IReadOnlyCollection<IReadOnlyCollection<BoardCoordinate>> CreateClassicCandidateShapes()
    {
        var shapes = new List<IReadOnlyCollection<BoardCoordinate>>();
        for (var length = 1; length <= 4; length++)
        {
            shapes.Add(Enumerable.Range(0, length).Select(column => new BoardCoordinate(0, column)).ToArray());
            if (length > 1)
                shapes.Add(Enumerable.Range(0, length).Select(row => new BoardCoordinate(row, 0)).ToArray());
        }

        return shapes;
    }

    private static IReadOnlyCollection<IReadOnlyCollection<BoardCoordinate>> CreatePaperCandidateShapes()
    {
        var shapes = new Dictionary<string, IReadOnlyCollection<BoardCoordinate>>();
        for (var length = 1; length <= 4; length++)
            foreach (var shape in GenerateConnectedShapes(length))
                shapes[OffsetSignature(shape)] = shape;

        return shapes.Values.ToArray();
    }

    private static IEnumerable<IReadOnlyCollection<BoardCoordinate>> GenerateConnectedShapes(int length)
    {
        var results = new Dictionary<string, IReadOnlyCollection<BoardCoordinate>>();
        var visited = new HashSet<string>();

        Expand([new BoardCoordinate(0, 0)]);

        return results.Values;

        void Expand(IReadOnlyCollection<BoardCoordinate> current)
        {
            var normalized = NormalizeOffsets(current).ToArray();
            var signature = OffsetSignature(normalized);
            if (!visited.Add(signature)) return;

            if (normalized.Length == length)
            {
                results[signature] = normalized;
                return;
            }
            ...
            foreach (var cell in normalized)
            {
                ...
                {
                    if (normalized.Contains(neighbor)) continue;
                    Expand(normalized.Append(neighbor).ToArray());
                }
            }
        }
    }

    private static IEnumerable<BoardCoordinate> TouchingCoordinates(BoardCoordinate cell)
    {
        for (var row = cell.Row - 1; row <= cell.Row + 1; row++)
        {
            for (var column = cell.Column - 1; column <= cell.Column + 1; column++)
                if (FleetRules.IsInside(row, column))
                    yield return new BoardCoordinate(row, column);
        }
    }

    private static IEnumerable<BoardCoordinate> OrthogonalNeighbors(BoardCoordinate cell)
    {
    ...
    private static string OffsetSignature(IEnumerable<BoardCoordinate> offsets) =>
        string.Join(';', NormalizeOffsets(offsets)
            .OrderBy(offset => offset.Row)
            .ThenBy(offset => offset.Column)
            .Select(offset => $"{offset.Row}:{offset.Column}"));

    private sealed record CandidatePlacement(
        int Length,
        HashSet<BoardCoordinate> Cells,
        HashSet<BoardCoordinate> TouchingCells);
